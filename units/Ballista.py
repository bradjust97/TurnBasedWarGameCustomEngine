...
from enums import BallistaEnums, FootmanEnums, Player, SquareBoard
...
class Ballista(Piece):
    def __init__(self, row_number, col_number, player):
        name = BallistaEnums.NAME
        movement = BallistaEnums.MOVEMENT
        super().__init__(name, row_number, col_number, player, movement)

    def get_valid_piece_takes(self, game_state):
        _moves = []
        possible_moves = make_movement_diamond(BallistaEnums.MOVEMENT)
    ...
    def get_valid_piece_moves(self, game_state):
        return blocked_movement_diamond(BallistaEnums.MOVEMENT, self.get_row_number(), self.get_col_number(), game_state)
